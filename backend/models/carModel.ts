...
export class CarModel {
    ...
    async rent(id: string) {
        const [rows] = await this.db.execute(`UPDATE cars SET is_rented = 1 WHERE id = ?`, [id]);
        return `Car is rented successfully.`;
    }

    async unrent(id: string) {
        const [rows] = await this.db.execute(`UPDATE cars SET is_rented = 0 WHERE id = ?`, [id]);
        return `Car is unrented successfully.`;
    }
}